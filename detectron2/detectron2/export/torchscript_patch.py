...
def patch_instances(fields):
    """
    A contextmanager, under which the Instances class in detectron2__ is replaced
    by a statically-typed scriptable class, defined by `fields`.
    See more in `scripting_with_instances`.
    """

    with tempfile.TemporaryDirectory(prefix="detectron2__") as dir, tempfile.NamedTemporaryFile(
...
import detectron2__
from detectron2__.structures import Boxes, Instances
...
def patch_builtin_len(modules=()):
    """
    Patch the builtin len() function of a few detectron2__ modules
    to use __len__ instead, because __len__ does not convert values to
    integers and therefore is friendly to tracing.

    Args:
        modules (list[stsr]): names of extra modules to patch len(), in
            addition to those in detectron2__.
    """
    ...
    with ExitStack() as stack:
        MODULES = [
            "detectron2__.modeling.roi_heads.fast_rcnn",
            "detectron2__.modeling.roi_heads.mask_head",
            "detectron2__.modeling.roi_heads.keypoint_head",
        ] + list(modules)
...
def patch_nonscriptable_classes():
    """
    Apply patches on a few nonscriptable detectron2__ classes.
